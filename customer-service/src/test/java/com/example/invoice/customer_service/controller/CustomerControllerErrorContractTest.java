...
class CustomerControllerErrorContractTest {
        ...
        @Test
        @DisplayName("an unsortable sort property is a 400, not a 500")
        void invalidSortIsBadRequest() throws Exception {
                // Thrown by JpaQueryTransformerSupport when the sort names something
                // absent from a declared @Query's select clause — CustomerRepository
                // .search is exactly that. Stubbed rather than provoked: the slice has
                // no database, and what is under test is the mapping, not Spring Data's
                // validation.
                given(customerService.listCustomers(any(), any()))
                                .willThrow(new InvalidDataAccessApiUsageException(
                                                "Sort expression '[\"string\"]: ASC' must only contain property "
                                                                + "references or aliases used in the select clause; "
                                                                + "please use JpaSort.unsafe(…)"));

                // The literal parameter Swagger UI prefills, which is how this reached
                // production as a 500 on the first Execute of any paged endpoint.
                mockMvc.perform(get("/api/customers").param("sort", "[\"string\"]"))
                                .andExpect(status().isBadRequest())
                                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
                                .andExpect(jsonPath("$.title").value("Invalid sort parameter"))
                                .andExpect(jsonPath("$.type")
                                                .value("https://invoice-generator/errors/invalid-sort"))
                                // The exception message names internal query structure and
                                // suggests JpaSort.unsafe. Neither belongs in a client response.
                                .andExpect(content().string(org.hamcrest.Matchers.not(
                                                org.hamcrest.Matchers.containsString("JpaSort"))));
        }
