 ...
 *
 * Every response carries a type URI, a title, a status and a human-readable
 * detail, so clients get one predictable error shape instead of Spring's
 * default page. Nothing here exposes a stack trace or an internal class name.
 */
...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleNotFound(InvoiceNotFoundException ex) {
        return problem(HttpStatus.NOT_FOUND, "Invoice not found", ex.getMessage(), "invoice-not-found");
    }

    @ExceptionHandler(InvalidCustomerException.class)
    public ProblemDetail handleInvalidCustomer(InvalidCustomerException ex) {
        return problem(HttpStatus.UNPROCESSABLE_ENTITY, "Unknown customer", ex.getMessage(), "unknown-customer");
    }
