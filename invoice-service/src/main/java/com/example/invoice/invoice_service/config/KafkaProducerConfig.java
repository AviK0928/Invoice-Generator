 ...
 *
 * The typed InvoiceEventDTO and ArchiveEventDTO templates are gone: nothing
 * publishes directly any more. Events are serialised to JSON when recorded in
 * the outbox and dispatched as pre-serialised strings, so the producer needs no
 * knowledge of the payload types.
 */
@Configuration
public class KafkaProducerConfig {
    ...
    public ProducerFactory<String, String> producerFactory() {
        ...
        // Fail fast rather than block. The dispatcher retries on its next poll,
        // so a long block here just stalls the batch behind an unreachable
        // broker — max.block.ms defaults to 60s.
